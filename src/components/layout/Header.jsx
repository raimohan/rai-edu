...
const Header = ({ notifications, setShowNotificationModal }) => {
    ...
    const { currentUser } = useAuth(); // <-- USE THE HOOK, GET currentUser
    ...
    // Use `currentUser` to get the initial
    ...
    return (
        <header className="bg-white p-4 shadow-md z-10 rounded-b-2xl flex items-center justify-between mx-4 mt-4 md:mx-auto md:max-w-full lg:max-w-full xl:max-w-full w-[calc(100%-2rem)] md:w-full">
            {/* You might want to pass the activeMenuItem prop from a parent component if needed */}
            ...
            <div className="flex items-center space-x-6">
                ...
                <div className="flex items-center space-x-3 cursor-pointer">
                    ...
                    {/* Use `currentUser` to get the display name */}
                    <span className="text-gray-700 font-medium hidden sm:block">{currentUser?.displayName || currentUser?.email}</span>
                </div>
            </div>
        </header>
    );
